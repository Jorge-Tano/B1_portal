...
export async function POST(request: NextRequest) {
  try {
    ...
    return await crearAnticipoIndividual(body);
  } catch (error: any) {
...
async function crearAnticipoIndividual(body: any) {
  try {
    const session = await getServerSession(authOptions);
    ...
    if (montoCheck.rows.length === 0) {
      const montosDisponiblesResult = await query('SELECT amount FROM amounts ORDER BY amount ASC');
      const montosPermitidos = montosDisponiblesResult.rows.map(row => row.amount);
