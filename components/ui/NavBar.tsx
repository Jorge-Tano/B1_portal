...
// Creamos un contexto o prop para compartir el estado
...
export function NavBar({ onExpandChange }: NavBarProps) {
  ...
  return (
    ...
    >
      {/* Contenido del NavBar (igual que antes) */}
      <div className={`h-full transition-all duration-400 ${
        isExpanded ? 'w-64' : 'w-3'
      }`}>
        
        <div className={`h-full bg-gradient-to-b hover:from-gray-400 from-gray-200 hover:to-gray-300 to-gray-200 border-r border-gray-400 transition-all duration-400 ${
          isExpanded ? 'shadow-xl' : ''
        }`}>
          
          {/* Puntos indicadores */}
          <div className={`absolute top-1/2 transition-all duration-400 ${
            isExpanded ? 'right-2' : 'right-1/2'
          }`} style={{ transform: 'translateY(-50%)' }}>
            <div className="flex flex-col items-center space-y-2">
              <div className={`w-1.5 h-1.5 rounded-full transition-colors ${
                isExpanded ? 'bg-violet-600' : 'bg-violet-500'
              }`}></div>
              <div className={`w-1.5 h-1.5 rounded-full transition-colors ${
                isExpanded ? 'bg-violet-500' : 'bg-violet-400'
              }`}></div>
              <div className={`w-1.5 h-1.5 rounded-full transition-colors ${
                isExpanded ? 'bg-violet-400' : 'bg-violet-300'
              }`}></div>
            </div>
          </div>
          ...
          {isExpanded && (
            <div className="h-full overflow-hidden">
              ...
              <div className="flex-1 p-5 mt-8 space-y-4">
                <a
                  href="/anticipos"
